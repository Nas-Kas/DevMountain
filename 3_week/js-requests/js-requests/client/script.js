...
const changeButtonProperties = () => {
    sayHelloButton.style.backgroundColor = "black";
    sayHelloButton.style.color = "white";
}

sayHelloButton.addEventListener('mouseover', changeButtonProperties);
...
repeatButton.addEventListener('click', () => {repeatMyParam('some string')});

const repeatMyParam = (someparam) => {
    axios.get(`${baseUrl}/repeat/${someparam}`)
    .then(response => {
        console.log(response.data);
...
queryBtn.addEventListener('click',getQueryTest);

const getQueryTest = () =>{
    axios.get(`${baseUrl}/query-test`)
    .then( response => {
        console.log(response.data)
        console.log("this is a query test")
    })
    .catch(err => console.log(err))
}
